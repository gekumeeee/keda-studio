...
// Contract line items each carry an optional amount; the total is their sum.
export function contractTotal(contract) {
  return (contract.items || []).reduce((sum, it) => sum + parseAmount(it.amount), 0);
}
