...
// Font family used everywhere below: 'Bricolage Grotesque' has no Arabic
// glyphs at all, so any Arabic the admin types (contract terms, party
// names, notes — all free text) rendered as broken/overlapping shapes.
// Cairo is the site's own Arabic font (see globals.css) and, critically,
// ships real GSUB/GPOS tables, which react-pdf's fontkit-based layout
// engine uses for proper contextual letter-joining — so listing it as a
// fallback (not a replacement) lets Latin text keep the branded
// Bricolage Grotesque look while Arabic runs automatically pick up Cairo.
const TEXT_FONT = ['Bricolage Grotesque', 'Cairo'];
...
function makeContractStyles(t, g) {
  ...
  return StyleSheet.create({
    ...
    clauseRow: { flexDirection: 'row', marginBottom: gp(8) },
    clauseNum: { width: ft(22), fontSize: ft(11), fontWeight: 800, color: ACCENT },
    clauseText: { flex: 1, fontSize: ft(11), color: INK, lineHeight: 1.5 },
    para: { fontSize: ft(11), color: INK, lineHeight: 1.5, marginBottom: gp(8) },
...
function estimateContractHeight(c, t, g) {
  ...
  if (clauses.length) {
    ...
    for (const t2 of clauses) {
      h += lineCount(t2, ft(11), CONTENT_WIDTH - ft(22)) * ft(11) * 1.5 + gp(8);
    }
  }
  ...
  if (notes.length) {
    ...
    for (const n of notes) {
      h += lineCount(n, ft(11), CONTENT_WIDTH) * ft(11) * 1.5 + gp(8);
    }
  }
